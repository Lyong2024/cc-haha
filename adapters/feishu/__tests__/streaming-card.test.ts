...
describe('StreamingCard: 错误处理', () => {
  ...
  it('CardKit 中间帧请求挂住时不会阻塞 message_complete 收尾', async () => {
    const previousTimeout = process.env.HAHA_IM_CARD_REQUEST_TIMEOUT_MS
    process.env.HAHA_IM_CARD_REQUEST_TIMEOUT_MS = '20'
    try {
      const { client, calls } = makeMockClient({
      ...
      if (previousTimeout === undefined) {
        delete process.env.HAHA_IM_CARD_REQUEST_TIMEOUT_MS
      } else {
        process.env.HAHA_IM_CARD_REQUEST_TIMEOUT_MS = previousTimeout
      }
    }
  })
})
