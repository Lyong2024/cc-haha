...
function getHahaDir(): string {
  const configDir =
    process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
  return path.join(configDir, 'haha')
}

function getFileBackedStorageMarkerPath(): string {
  return path.join(getHahaDir(), FILE_BACKED_STORAGE_MARKER_FILE)
}

function markFileBackedStorageUsed(): void {
  try {
    fs.mkdirSync(getHahaDir(), { recursive: true })
