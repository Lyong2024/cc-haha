...
async function readDesktopUiFile(): Promise<Record<string, unknown>> {
  const raw = await fs.readFile(path.join(tmpDir, 'haha', 'desktop-ui.json'), 'utf-8')
...
describe('DesktopUiPreferencesService', () => {
  ...
  test('returns defaults when desktop-ui.json does not exist', async () => {
    ...
    expect(result.preferences).toEqual({
      ...
      profile: {
        displayName: 'haha',
        subtitle: 'github.com/NanmiCoder/__KEEP_HAHA__',
        avatarFile: null,
  ...
  test('normalizes old schema files and preserves unknown fields when updating sidebar preferences', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'haha', 'desktop-ui.json'),
      JSON.stringify({
        schemaVersion: 2,
  ...
  test('quarantines corrupt desktop-ui.json and reports defaults as missing', async () => {
    await fs.mkdir(path.join(tmpDir, 'haha'), { recursive: true })
    await fs.writeFile(path.join(tmpDir, 'haha', 'desktop-ui.json'), '{bad json', 'utf-8')
    ...
    expect(result.preferences.profile.displayName).toBe('haha')
  ...
  test('stores uploaded profile avatars under haha profile storage', async () => {
    const service = new DesktopUiPreferencesService()
    ...
    const avatar = await fs.readFile(path.join(tmpDir, 'haha', 'profile', 'avatar.png'))
  ...
  test('clears only managed profile avatar files', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, 'haha', 'profile', 'local-note.txt'), 'keep me', 'utf-8')
    ...
    await expect(fs.readFile(path.join(tmpDir, 'haha', 'profile', 'avatar.png'))).rejects.toThrow()
    await expect(fs.readFile(path.join(tmpDir, 'haha', 'profile', 'local-note.txt'), 'utf-8')).resolves.toBe('keep me')
  })
...
describe('desktop UI preferences API', () => {
  ...
  test('persists sidebar preferences under haha desktop-ui.json', async () => {
