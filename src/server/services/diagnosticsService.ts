...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'haha', 'diagnostics')
  }
  ...
  async recordEvent(input: DiagnosticEventInput): Promise<DiagnosticWriteResult> {
    ...
    // ~/.claude/haha/diagnostics. Tests that genuinely exercise diagnostics
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `haha-diagnostics-${timestamp}.tar.gz`
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'haha', 'providers.json')
    try {
