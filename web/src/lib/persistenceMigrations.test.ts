...
describe('desktop persistence migrations', () => {
  ...
  test('migrates legacy open-tab arrays into the current tab persistence shape', () => {
    window.localStorage.setItem('haha-open-tabs', JSON.stringify([
    ...
    expect(report.migratedKeys).toContain('haha-open-tabs')
    expect(JSON.parse(window.localStorage.getItem('haha-open-tabs') || '{}')).toEqual({
  ...
  test('filters stale session runtime selections without clearing unrelated keys', () => {
    ...
    window.localStorage.setItem('haha-session-runtime', JSON.stringify({
    ...
    expect(JSON.parse(window.localStorage.getItem('haha-session-runtime') || '{}')).toEqual({
      alsoGood: { providerId: 'openai-official', modelId: 'gpt-5.6-sol', effortLevel: 'xhigh' },
      good: { providerId: null, modelId: 'claude-sonnet' },
    })
  ...
  test('removes malformed known keys without throwing during startup', () => {
    window.localStorage.setItem('haha-open-tabs', '{"openTabs":')
    window.localStorage.setItem('haha-theme', 'sepia')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('haha-open-tabs')
    expect(report.migratedKeys).toContain('haha-theme')
    expect(window.localStorage.getItem('haha-open-tabs')).toBeNull()
    expect(window.localStorage.getItem('haha-theme')).toBeNull()
  })

  test('preserves the pure white theme as a valid persisted theme', () => {
    window.localStorage.setItem('haha-theme', 'white')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).not.toContain('haha-theme')
    expect(window.localStorage.getItem('haha-theme')).toBe('white')
  })
  ...
  test('renames the retired light theme to warm-classic instead of resetting it', () => {
    ...
    window.localStorage.setItem('haha-theme', 'light')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('haha-theme')
    expect(window.localStorage.getItem('haha-theme')).toBe('warm-classic')
  })
  ...
  test('applies the same rename to the light half of follow-the-system', () => {
    ...
    window.localStorage.setItem('haha-light-theme', 'light')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('haha-light-theme')
    expect(window.localStorage.getItem('haha-light-theme')).toBe('warm-classic')
  })

  test('preserves every palette introduced by the redesign', () => {
    for (const theme of ['white', 'paper', 'warm-classic', 'celadon', 'dark', 'ink-blue']) {
      window.localStorage.setItem('haha-theme', theme)

      const report = runDesktopPersistenceMigrations()

      expect(report.migratedKeys, `${theme} should survive startup migration`).not.toContain('haha-theme')
      expect(window.localStorage.getItem('haha-theme')).toBe(theme)
    }
  })
  ...
  test('drops a malformed follow-the-system flag rather than reading it as opted in', () => {
    ...
    window.localStorage.setItem('haha-follow-system-theme', 'yes')
    // A dark palette is not a valid light half, and vice versa.
    window.localStorage.setItem('haha-light-theme', 'ink-blue')
    window.localStorage.setItem('haha-dark-theme', 'celadon')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('haha-follow-system-theme')
    expect(report.migratedKeys).toContain('haha-light-theme')
    expect(report.migratedKeys).toContain('haha-dark-theme')
    expect(window.localStorage.getItem('haha-follow-system-theme')).toBeNull()
    expect(window.localStorage.getItem('haha-light-theme')).toBeNull()
    expect(window.localStorage.getItem('haha-dark-theme')).toBeNull()
  })

  test('preserves a valid follow-the-system flag and both ground preferences', () => {
    window.localStorage.setItem('haha-follow-system-theme', '1')
    window.localStorage.setItem('haha-light-theme', 'celadon')
    window.localStorage.setItem('haha-dark-theme', 'ink-blue')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).not.toContain('haha-follow-system-theme')
    expect(report.migratedKeys).not.toContain('haha-light-theme')
    expect(report.migratedKeys).not.toContain('haha-dark-theme')
    expect(window.localStorage.getItem('haha-follow-system-theme')).toBe('1')
    expect(window.localStorage.getItem('haha-light-theme')).toBe('celadon')
    expect(window.localStorage.getItem('haha-dark-theme')).toBe('ink-blue')
  })

  test('preserves every supported locale during startup migration', () => {
    for (const locale of ['en', 'zh', 'zh-TW', 'jp', 'kr']) {
      window.localStorage.setItem('haha-locale', locale)

      const report = runDesktopPersistenceMigrations()

      expect(report.migratedKeys).not.toContain('haha-locale')
      expect(window.localStorage.getItem('haha-locale')).toBe(locale)
    }
  })

  test('preserves valid app zoom and removes invalid app zoom values', () => {
    window.localStorage.setItem('haha-app-zoom', '1.2')

    const validReport = runDesktopPersistenceMigrations()

    expect(validReport.migratedKeys).not.toContain('haha-app-zoom')
    expect(window.localStorage.getItem('haha-app-zoom')).toBe('1.2')

    window.localStorage.setItem('haha-app-zoom', '4')

    const invalidReport = runDesktopPersistenceMigrations()

    expect(invalidReport.migratedKeys).toContain('haha-app-zoom')
    expect(window.localStorage.getItem('haha-app-zoom')).toBeNull()
  })

  test('migrates the legacy UI zoom key into app zoom storage', () => {
    window.localStorage.setItem('haha-ui-zoom', '1.25')
    ...
    expect(window.localStorage.getItem('haha-app-zoom')).toBe('1.25')
    expect(window.localStorage.getItem('haha-ui-zoom')).toBeNull()
  })
