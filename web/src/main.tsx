...
declare global {
  interface Window {
    __HAHA_BOOTSTRAPPED__?: boolean
    __HAHA_SHOW_STARTUP_ERROR__?: (reason: unknown) => void
  }
}
...
/** Pet windows are removed on the pure-web product path. */
export function isPetWindowLocation(_search = window.location.search): boolean {
  return false
}
...
export async function bootstrapDesktopApp(
  ...
  try {
    ...
    window.__HAHA_BOOTSTRAPPED__ = true
  } catch (error) {
    console.error('[desktop] Failed to bootstrap app', error)
    if (root) {
      if (window.__HAHA_SHOW_STARTUP_ERROR__) {
        window.__HAHA_SHOW_STARTUP_ERROR__(error)
      } else {
