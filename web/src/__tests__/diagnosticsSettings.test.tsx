...
describe('Settings > Diagnostics tab', () => {
  beforeEach(() => {
    ...
    diagnosticsApiMock.getStatus.mockResolvedValue({
      logDir: '/tmp/claude/haha/diagnostics',
      diagnosticsPath: '/tmp/claude/haha/diagnostics/diagnostics.jsonl',
      cliDiagnosticsPath: '/tmp/claude/haha/diagnostics/cli-diagnostics.jsonl',
      runtimeErrorsPath: '/tmp/claude/haha/diagnostics/runtime-errors.log',
      exportDir: '/tmp/claude/haha/diagnostics/exports',
      retentionDays: 7,
    ...
    diagnosticsApiMock.exportBundle.mockResolvedValue({
      bundle: {
        path: '/tmp/claude/haha/diagnostics/exports/haha-diagnostics.tar.gz',
        fileName: 'haha-diagnostics.tar.gz',
        bytes: 1024,
      },
    })
        ...
        items: [
          {
            ...
            kind: 'json',
            scope: 'user',
            path: '~/.claude/haha/providers.json',
            protected: true,
            exists: true,
  ...
  it('shows diagnostics status, actions, and recent events', async () => {
    ...
    expect(screen.getByText('/tmp/claude/haha/diagnostics')).toBeInTheDocument()
  ...
  it('lets a pending rebuild win over an export refresh and always releases its busy state', async () => {
    ...
    await act(async () => {
      exportRequest.resolve({
        bundle: {
          path: '/tmp/claude/haha/diagnostics/exports/race.tar.gz',
  ...
  it('keeps legacy diagnostics usable when the additive local-index endpoint is unavailable', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/haha/diagnostics')).toBeInTheDocument()
  ...
  it('exports a diagnostics bundle from the settings page', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/haha/diagnostics/exports/haha-diagnostics.tar.gz')).toBeInTheDocument()
  })
  ...
  it('checks findings first and confirms before resetting only safe desktop state', async () => {
    ...
    window.localStorage.setItem('haha-chat-history', 'keep')
    ...
    expect(window.localStorage.getItem('haha-theme')).toBe('haha-theme-value')
    expect(screen.getByText('~/.claude/haha/providers.json')).toBeInTheDocument()
    ...
    expect(screen.getByText(/haha-app-zoom/)).toBeInTheDocument()
    expect(screen.getByText(/haha-ui-zoom/)).toBeInTheDocument()
    ...
    expect(window.localStorage.getItem('haha-chat-history')).toBe('keep')
    expect(screen.getByText(/Removed keys:.*haha-app-zoom/)).toBeInTheDocument()
  })
  ...
  it('clears an existing Doctor report when the active cwd changes', async () => {
    ...
    fireEvent.click(screen.getByText('Diagnostics'))
    fireEvent.click(await screen.findByRole('button', { name: /Run Doctor/i }))
    expect(await screen.findByText('~/.claude/haha/providers.json')).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(screen.queryByText('~/.claude/haha/providers.json')).not.toBeInTheDocument()
    })
  ...
  it('preserves reset results and releases reset loading when cwd changes during refresh', async () => {
    ...
    expect(await screen.findByText(/Removed keys:.*haha-app-zoom/)).toBeInTheDocument()
