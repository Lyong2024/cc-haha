...
const {
  ...
  MOCK_DELETE_PROVIDER: vi.fn(),
  MOCK_GET_SETTINGS: vi.fn(),
  MOCK_UPDATE_SETTINGS: vi.fn(),
}))
...
vi.mock('../api/providers', () => ({
  providersApi: {
    ...
    syncQuota: vi.fn().mockResolvedValue({ provider: { id: 'grok-official' } }),
    refreshCredential: vi.fn().mockResolvedValue({ provider: { id: 'grok-official' }, refreshed: true }),
    exportCredentials: vi.fn().mockResolvedValue({ kind: 'api_key', filename: 'x.json' }),
    rename: vi.fn().mockResolvedValue({ provider: { id: 'grok-official' } }),
  },
}))

vi.mock('../components/settings/ProviderAccountPanel', () => ({
  ProviderAccountPanel: () => <div data-testid="provider-account-panel" />,
}))
...
describe('Settings > General tab', () => {
  beforeEach(() => {
    ...
    tauriDialogMock.open.mockResolvedValue('/Users/test/haha-data')
    ...
    useSettingsStore.setState({
      ...
      traceCapture: { enabled: true, storageDir: '/Users/test/.claude/haha/traces' },
  ...
  it('lets desktop users choose a custom data directory and relaunch immediately', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Custom data directory')).toHaveValue('/Users/test/haha-data')
    })
    ...
    await waitFor(() => {
      expect(useSettingsStore.getState().setAppMode).toHaveBeenCalledWith('portable', '/Users/test/haha-data')
  ...
  it('switches back to ~/.claude without deleting custom data', async () => {
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
        portableDir: '/Users/test/haha-data',
        activeConfigDir: '/Users/test/haha-data',
...
describe('Settings > Providers tab', () => {
  ...
  it('shows official OAuth status only after official provider is confirmed active', () => {
    ...
    // Official login is collapsed by default; click card header to expand.
    expect(screen.queryByTestId('claude-official-login')).not.toBeInTheDocument()
    const claudeToggle = screen.getByTestId('provider-card-toggle-claude-official')
  ...
  it('shows ChatGPT Official as the active built-in provider', () => {
    ...
    expect(screen.queryByTestId('chatgpt-official-login')).not.toBeInTheDocument()
    fireEvent.click(screen.getByTestId('provider-card-toggle-openai-official'))
  ...
  it('shows Grok Official as the active built-in provider', () => {
    ...
    // Grok account tools are collapsible; click card header to expand/collapse.
    expect(provider).toHaveAttribute('data-expanded', 'false')
    expect(screen.queryByTestId('grok-official-login')).not.toBeInTheDocument()
    const toggle = screen.getByTestId('provider-card-toggle-grok-official')
    fireEvent.click(toggle)
    expect(provider).toHaveAttribute('data-expanded', 'true')
