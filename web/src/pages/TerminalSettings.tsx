...
import { openHostTerminal } from '../api/hostTerminal'
...
export function TerminalSettings({
  ...
  const [hostOpenBusy, setHostOpenBusy] = useState(false)
  const [hostOpenMessage, setHostOpenMessage] = useState<string | null>(null)
  const isWindows = typeof navigator !== 'undefined' && /Win/i.test(navigator.platform || navigator.userAgent)
  const embeddedTerminalAvailable = isDesktopRuntime() && terminalApi.isAvailable()
  ...
  // Embedded PTY only exists in desktop runtime. Pure-web uses host terminal open.
  const hasTerminalPanel = embeddedTerminalAvailable && status !== 'unavailable'
  ...
  const openHostDefaultTerminal = async () => {
    setHostOpenBusy(true)
    setHostOpenMessage(null)
    try {
      const result = await openHostTerminal(cwd)
      setHostOpenMessage(result.message)
    } catch (err) {
      setHostOpenMessage(err instanceof Error ? err.message : String(err))
    } finally {
      setHostOpenBusy(false)
    }
  }
      ...
      >
        ...
        >
          <div className="flex min-w-0 flex-1 items-center gap-2.5">
            ...
            >
              {embeddedTerminalAvailable ? t('settings.terminal.title') : '宿主机终端'}
            </h2>
            {shellInfo && (
          ...
          <div className="flex shrink-0 items-center gap-0.5">
            {embeddedTerminalAvailable ? (
              ...
              <Button
                ...
                size="sm"
                variant="secondary"
                loading={hostOpenBusy}
                onClick={() => void openHostDefaultTerminal()}
              >
                打开默认终端
              </Button>
            )}
        ...
        {!embeddedTerminalAvailable ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 px-5 py-10 text-center">
            <span className="material-symbols-outlined text-[28px] text-[var(--color-text-tertiary)]" aria-hidden="true">
              terminal
            </span>
            <div className="text-sm font-semibold text-[var(--color-text-primary)]">在宿主机打开终端</div>
            <p className="max-w-md text-xs text-[var(--color-text-secondary)]">
              {cwd
                ? `以 Web Server 进程用户权限打开系统默认终端，工作目录：${cwd}`
                : '以 Web Server 进程用户权限打开系统默认终端（Windows Terminal / PowerShell / Terminal.app 等）。'}
            </p>
            <Button
              type="button"
              size="sm"
              loading={hostOpenBusy}
              disabled={hostOpenBusy}
              onClick={() => void openHostDefaultTerminal()}
            >
              {hostOpenBusy ? '正在打开…' : '打开默认终端'}
            </Button>
            {hostOpenMessage ? (
              <p className="max-w-md text-xs text-[var(--color-text-tertiary)]">{hostOpenMessage}</p>
            ) : null}
          </div>
        ) : status === 'unavailable' ? (
          <EmptyState
