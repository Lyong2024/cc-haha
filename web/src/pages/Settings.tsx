...
import { ChevronDown, ChevronRight, Copy, Eye, EyeOff, GripVertical, PowerOff, QrCode, RotateCw } from 'lucide-react'
...
import { ProviderAccountPanel } from '@/components/settings/ProviderAccountPanel'
import { ExternalProviderImportPanel } from '@/components/settings/ExternalProviderImportPanel'
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[220px] flex-shrink-0 flex flex-col overflow-y-auto border-r border-[var(--color-border)] bg-[var(--color-surface-container-low)] px-3 py-4">
          <div className="flex-1 flex flex-col gap-0.5">
            ...
            <TabButton icon="admin_panel_settings" label="系统管理" active={activeTab === 'system'} onClick={() => setActiveTab('system')} />
...
function ProviderSettings() {
  ...
  return (
    <div className="w-full min-w-0 max-w-[min(100%,72rem)]">
      <SettingsPageHeader
      ...
      <ExternalProviderImportPanel onImported={() => void fetchProviders()} />
          ...
          <div className="flex w-full min-w-0 flex-col gap-2">
            {providerItems.map((item) => {
              if (item.kind === 'claude-official') {
                return (
                  <SortableProviderCard
                    ...
                    // Official login only — collapse by default to save vertical space.
                    collapsibleDetails={isClaudeOfficialActive}
                    detailsLabel="官方登录"
              ...
              if (item.kind === 'openai-official') {
                return (
                  <SortableProviderCard
                    ...
                    collapsibleDetails={isOpenAIOfficialActive}
                    detailsLabel="官方登录"
              ...
              if (item.kind === 'grok-official') {
                const grokLoggedIn = !!grokOAuthStatus?.loggedIn
                return (
                  <SortableProviderCard
                    key={item.id}
                    item={item}
                    ...
                    dragLabel={t('settings.providers.dragToReorder')}
                    // Same as Claude/OpenAI: click inactive card → set as default provider.
                    // (Account-pool “设为默认” is a separate sticky-account action inside the panel.)
                    ...
                    badges={(
                      <>
                        {isGrokOfficialActive ? (
                          <Badge tone="brand" bordered>{t('settings.providers.default')}</Badge>
                        ) : null}
                        {grokLoggedIn ? (
                          <Badge tone="success" size="sm">OAuth</Badge>
                        ) : (
                          <Badge tone="neutral" size="sm">未登录</Badge>
                        )}
                      </>
                    )}
                    actions={!isGrokOfficialActive ? (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => void handleActivate(GROK_OFFICIAL_PROVIDER_ID)}
                      >
                        {t('settings.providers.setDefault')}
                      </Button>
                    ) : null}
                    // Expandable account/quota panel (login or multi-account pool).
                    // Right rail: 设为默认 then expand chevron (see SortableProviderCard).
                    collapsibleDetails
                    detailsLabel={grokLoggedIn ? '账号与额度' : '官方登录'}
                    detailsDefaultOpen={false}
                    details={(
                      <div>
                        {/* Login UI only when not logged in; when logged in, pool table is the account list. */}
                        {!grokLoggedIn ? (
                          <div className="px-4 pb-2 pt-1">
                            <GrokOfficialLogin />
                          </div>
                        ) : null}
                        {/* Multi-account pool is Grok Official only — never mount on other cards. */}
                        {grokLoggedIn ? (
                          <ProviderAccountPanel
                            provider={GROK_OFFICIAL_PROVIDER_SHELL}
                            isActive={isGrokOfficialActive}
                            officialOAuth
                            flush
                            autoSync
              ...
              // Third-party / custom API providers: compact card only.
              // Account quota / auth.json export / credential refresh are official-OAuth features.
...
type SortableProviderCardProps = {
  ...
  details?: ReactNode
  /**
   * When true, details start collapsed.
   * Click the card header (not drag handle / action buttons) to expand or collapse.
   */
  collapsibleDetails?: boolean
  /** Default open state for collapsible details (default: false). */
...
function SortableProviderCard({
  ...
  const canToggleDetails = hasDetails && collapsibleDetails
  const showDetailsBody = hasDetails && (!collapsibleDetails || detailsOpen)

  /**
   * Header click priority (align Claude / OpenAI / Grok / custom):
   * 1. Inactive + onActivate → set as default provider (even if collapsible)
   * 2. Else collapsible → expand / collapse details
   * 3. Else onActivate
   *
   * Previously Grok always collapsible and expand swallowed activate, so users
   * had to press a separate “设为默认” while OpenAI/Claude activated on card click.
   */
  ...
  const headerInteractive = canToggleDetails || !!onActivate
  const expandLabel = detailsOpen ? `收起${detailsLabel}` : `展开${detailsLabel}`

  const toggleDetailsOnly = (e?: { stopPropagation?: () => void }) => {
    e?.stopPropagation?.()
    if (canToggleDetails) {
      setDetailsOpen((open) => !open)
    }
  ...
  return (
    <div
      ...
      data-expanded={canToggleDetails ? detailsOpen : undefined}
      className={`group relative flex flex-col rounded-[var(--radius-xl)] transition-[background-color,border-color,box-shadow,transform] duration-150 ease-out ${
        isActive
        ...
        canToggleDetails && detailsOpen
          ? 'shadow-[var(--shadow-sm,0_1px_2px_rgb(0_0_0/0.06))]'
          : ''
      }`}
    >
      {/*
        Header rail (LTR):
        [drag] [title…] ………… [actions: 设为默认 …] [chevron SVG]
        Actions + expand chevron are a right-aligned cluster; chevron is rightmost.
      */}
      <div
        className={`flex items-center gap-2 px-3.5 py-3 select-none ${
          canToggleDetails && detailsOpen ? 'rounded-t-[var(--radius-xl)] bg-[var(--color-surface-container-low)]/60' : ''
        }`}
      >
        <button
          type="button"
        ...
        <button
          ...
          aria-disabled={!headerInteractive}
          className={`flex min-w-0 flex-1 items-center gap-3 rounded-[var(--radius-sm)] text-left transition-[background-color,transform,box-shadow] duration-150 focus:outline-none focus-visible:shadow-[var(--shadow-focus-ring)] ${
            headerInteractive
              ? 'cursor-pointer hover:bg-[var(--color-surface-container-high)]/50 active:scale-[0.995] active:bg-[var(--color-surface-container-high)]'
              : 'cursor-default'
          }`}
        >
        ...
        {/* Right-aligned: 设为默认 (and other actions) then expand SVG */}
        <div
          className="ml-auto flex shrink-0 items-center justify-end gap-1"
          data-testid={`provider-card-actions-${item.id}`}
        >
          {actions ? (
            <div
          ...
          {canToggleDetails ? (
            <button
              type="button"
              data-testid={`provider-card-toggle-${item.id}`}
              onClick={toggleDetailsOnly}
              aria-expanded={detailsOpen}
              aria-label={expandLabel}
              title={expandLabel}
              className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[var(--color-text-tertiary)] transition-[background-color,color,transform] duration-150 focus:outline-none focus-visible:shadow-[var(--shadow-focus-ring)] ${
                detailsOpen
                  ? 'bg-[var(--color-surface-container-high)] text-[var(--color-text-secondary)]'
                  : 'hover:bg-[var(--color-surface-container-high)] hover:text-[var(--color-text-secondary)] group-hover:bg-[var(--color-surface-container-high)] group-hover:text-[var(--color-text-secondary)]'
              }`}
            >
              {detailsOpen ? (
      ...
      {showDetailsBody ? (
        <div
          className="overflow-visible border-t border-[var(--color-border-separator)] pb-1"
          data-testid={canToggleDetails ? `provider-card-details-${item.id}` : undefined}
        >
          {details}
        </div>
      ) : null}
...
/** Preferred order in the "Add Provider" modal pill list. */
const ADD_PROVIDER_PRESET_PRIORITY: Record<string, number> = {
  claude: 0,
  openai: 1,
  deepseek: 2,
  // Keep local + custom at the end relative to third-party cloud presets.
  lmstudio: 900,
  ollama: 901,
  custom: 999,
}

function sortAddProviderPresets(presets: ProviderPreset[]): ProviderPreset[] {
  return [...presets].sort((a, b) => {
    const ra = ADD_PROVIDER_PRESET_PRIORITY[a.id] ?? 100
    const rb = ADD_PROVIDER_PRESET_PRIORITY[b.id] ?? 100
    if (ra !== rb) return ra - rb
    // Stable-ish for equal priority: keep original relative order via name.
...
function ProviderFormModal({ open, onClose, mode, provider, presets }: ProviderFormProps) {
  ...
  // Built-in OAuth shell "official" is managed by the provider list cards, not the add modal.
  // Display priority: Claude → OpenAI → DeepSeek → others → Custom last.
  const loadedPresets = sortAddProviderPresets(presets.filter((p) => p.id !== 'official'))
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Write the edited haha settings.json first so provider-specific model
...
const GITHUB_REPO = 'https://github.com/NanmiCoder/__KEEP_HAHA__'
      ...
      <div className="mt-6 w-full">
        ...
        >
          ...
          <div className="flex-1 text-left">
            <div className="text-sm font-medium text-[var(--color-text-primary)]">NanmiCoder/haha</div>
