...
describe('uiStore following the system appearance', () => {
  ...
  it('freezes the theme on screen when the switch is turned off', async () => {
    ...
    useUIStore.getState().setFollowSystemTheme(false)
    expect(useUIStore.getState().theme).toBe('dark')
    expect(window.localStorage.getItem('haha-theme')).toBe('dark')
    expect(window.localStorage.getItem('haha-follow-system-theme')).toBe('0')
    teardownTheme()
  })
  ...
  it('does not revive the switch from a window whose store went stale', async () => {
    ...
    window.localStorage.setItem('haha-follow-system-theme', '0')
  ...
  it('catches up when another window changes the appearance', async () => {
    ...
    window.localStorage.setItem('haha-theme', 'celadon')
    window.localStorage.setItem('haha-light-theme', 'celadon')
    window.dispatchEvent(new StorageEvent('storage', { key: 'haha-light-theme' }))
  ...
  it('ignores storage events for unrelated keys', async () => {
    ...
    window.localStorage.setItem('haha-theme', 'celadon')
    window.dispatchEvent(new StorageEvent('storage', { key: 'haha-open-tabs' }))
...
describe('uiStore settings tab persistence', () => {
  ...
  it('hydrates the last selected Settings tab after the renderer store is recreated', async () => {
    ...
    expect(window.localStorage.getItem('haha-active-settings-tab')).toBe('general')
  ...
  it('persists the pets Settings tab', async () => {
    ...
    expect(window.localStorage.getItem('haha-active-settings-tab')).toBe('pets')
  ...
  it('ignores an invalid persisted Settings tab', async () => {
    window.localStorage.setItem('haha-active-settings-tab', 'not-a-settings-tab')
