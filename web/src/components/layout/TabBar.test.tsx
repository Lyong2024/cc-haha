...
describe('TabBar', () => {
  ...
  it('keeps the desktop tab strip at a roomier titlebar height', async () => {
    ...
    // Strip + drag gutter stay at the roomy 52px titlebar height. Tabs sit
    // slightly shorter (Chrome-style rounded tops with a strip gap above).
    expect(tabBar).toHaveClass('min-h-[52px]')
    expect(tab).toHaveClass('min-h-[46px]')
    expect(tab).toHaveClass('self-end')
  ...
  it('lifts the active tab onto the paper ground without turning it into a pill', async () => {
    ...
    // The fill is the point: strip stays on sidebar ground, active tab is
    // paper so it lifts off the desk and continues into the content view.
    ...
    // Chrome-style frame: top rounded corners + side/top border on active.
    // Not a floating pill — only top radius, open bottom (border-b-0), and
    // the brand underline highlight remains the selection accent.
    expect(active?.className).toMatch(/rounded-t-/)
    expect(active?.className).toContain('border-b-0')
    expect(active?.className).toContain('border-[var(--color-border)]')
