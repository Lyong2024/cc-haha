...
vi.mock('./HostShellPanel', () => ({
  HostShellPanel: () => <div data-testid="host-shell-panel-stub">host shell</div>,
}))

// Pure-web no longer mounts H5ConnectionView; keep mock only for legacy import safety.
...
vi.mock('../doctor/DoctorPanel', () => ({
  DoctorPanel: () => <div data-testid="doctor-panel-stub">doctor</div>,
}))
...
describe('AppShell boot flow', () => {
  ...
  it('does not auto-sync pet preferences on pure-web (no desktop pet host)', async () => {
    ...
    await screen.findByText('sidebar loaded')
    // Pure-web intentionally skips pet auto-show / lastSession sync.
    await waitFor(() => {
      expect(mocks.updatePetPreferences).not.toHaveBeenCalled()
    })
  })
  ...
  it('routes pet session navigation without auto-showing the pet on pure-web', async () => {
    mocks.isTauriRuntime = true
    ...
    await screen.findByText('sidebar loaded')
    // Pure-web: no pet auto-show on boot.
  ...
  it('shows startup error view for H5/auth bootstrap failures (pure-web uses AdminAuthGate)', async () => {
  ...
  it('shows startup error for unreachable remote bootstrap failures', async () => {
    mocks.initializeDesktopServerUrl.mockRejectedValueOnce(
    ...
    expect(await screen.findByText('app.serverFailed')).toBeInTheDocument()
    expect(screen.getByText(/Unable to reach https:\/\/remote\.example\.com/)).toBeInTheDocument()
    expect(screen.queryByText('h5 connection view')).not.toBeInTheDocument()
  })

  it('exposes a retry control on the pure-web startup error view', async () => {
    const reload = vi.fn()
    ...
    // Pure-web StartupErrorView reloads the page to re-enter AdminAuthGate/bootstrap.
    expect(reload).toHaveBeenCalled()
  })

  it('shows startup error view for desktop runtime bootstrap failures', async () => {
    mocks.isTauriRuntime = true
    mocks.initializeDesktopServerUrl.mockRejectedValueOnce(
