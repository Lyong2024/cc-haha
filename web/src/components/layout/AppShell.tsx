  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        // Pure-web: no desktop pet host. Skip pet auto-show entirely.
        ...
        if (!cancelled) {
          // Pure-web uses AdminAuthGate; do not route into H5 token connect UI.
  ...
  return (
    <div className={`app-shell app-shell-viewport flex overflow-hidden bg-[var(--color-surface)]${isMobileShell ? ' app-shell--mobile' : ''}`}>
      ...
      >
        ...
        {/*
          ContentRouter uses absolute-inset page panels; it must be a flex-1
          child of this column so it gets a real height. A non-flex intermediate
          wrapper collapses that height to 0 and blanks every tab surface.
        */}
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          <ContentRouter />
          {!isMobileShell ? <HostShellPanel /> : null}
        </div>
      </main>
      <ToastContainer />
      <UpdateChecker />
