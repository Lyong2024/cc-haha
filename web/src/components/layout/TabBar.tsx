...
import { useHostShellStore } from '../../stores/hostShellStore'
...
export function TabBar() {
  ...
  const isHostShellOpen = useHostShellStore((state) => state.open)
  ...
  return (
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1 border-l border-[var(--color-border)] px-2">
        {!isDesktopRuntime ? <CliLogoSwitcher /> : null}
        ...
        <IconButton
          ...
          onClick={() => {
            // Pure-web: open in-page host shell (bottom panel) connected to
            // the server OS user's default shell via PTY WebSocket.
            if (!isDesktopRuntime) {
              const cwd = openProjectPath
              useHostShellStore.getState().toggle(cwd)
              return
            }
            if (activeTabId && isActiveSessionTab) {
          ...
          size="md"
          tone={
            (!isDesktopRuntime && isHostShellOpen) || isTerminalPanelOpen
              ? 'default'
              : 'muted'
          }
          pressed={isDesktopRuntime ? isTerminalPanelOpen : isHostShellOpen}
          data-active={
            (isDesktopRuntime ? isTerminalPanelOpen : isHostShellOpen) ? 'true' : 'false'
          }
        />
...
const TabItem = forwardRef<HTMLDivElement, {
  ...
  return (
    <div
      ...
      className={`
        tab-bar-interactive group relative flex min-h-[46px] min-w-[140px] max-w-[200px] flex-shrink-0 items-center gap-1.5 self-end px-3
        mx-0.5 rounded-t-[10px] border border-b-0
        ...
        ${isActive
          // Chrome-style active tab: paper fill, top rounded corners, side/top
          // border frame. Keep the brand underline highlight (inset rule).
          // Bottom stays open (border-b-0) so the tab reads continuous with
          // content — not a floating pill with four rounded corners.
          ? 'z-[1] border-[var(--color-border)] bg-[var(--color-surface)] shadow-[inset_0_-3px_0_var(--color-brand)]'
          // Inactive: same border box (transparent) to avoid layout shift.
          // Hover shows a softer frame + paper, still without the brand rule.
