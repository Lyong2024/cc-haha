...
describe('WorkspacePanel', () => {
  ...
  it('keeps navigator controls and filter feedback in one header when no file is open', async () => {
    ...
    await setWorkspaceState((state) => ({
      ...
      statusBySession: {
        ...
        [sessionId]: {
          ...
          isGitRepo: true,
          changedFiles: [
            { path: 'web/src/App.tsx', status: 'modified', additions: 7, deletions: 2 },
            { path: 'web/src/theme.css', status: 'modified', additions: 3, deletions: 1 },
    ...
    expect(view.container.querySelector('[data-workspace-file-path="web/src/theme.css"]')).toBeTruthy()
    expect(view.container.querySelector('[data-workspace-file-path="web/src/App.tsx"]')).toBeNull()
  ...
  it('groups changed files by directory without weakening file filtering', async () => {
    ...
    expect(view.getByText('web/src')).toBeTruthy()
  ...
  it('gives renamed files enough height to show the old path without overlapping the next row', async () => {
    ...
    await setWorkspaceState((state) => ({
      ...
      statusBySession: {
        ...
        [sessionId]: {
          ...
          changedFiles: [
            {
              path: 'web/src/components/workspace/NewWorkspacePanel.tsx',
              oldPath: 'web/src/components/workspace/LegacyWorkspacePanelWithALongName.tsx',
              status: 'renamed',
              additions: 2,
              deletions: 2,
            },
            { path: 'web/src/components/workspace/next.ts', status: 'modified', additions: 1, deletions: 0 },
          ],
        },
      },
    }))
    ...
    const view = await renderPanel(sessionId)
    const oldPath = view.getByText('web/src/components/workspace/LegacyWorkspacePanelWithALongName.tsx')
