...
describe('MarkdownRenderer', () => {
  ...
  it('does not treat table/currency pseudo-math as formulas', () => {
    const { container } = render(
      <MarkdownRenderer
        content={'成本 $0.0058 | 输入 | **$0.022** 合计\n\n单价 $3/百万 token'}
      />,
    )
    expect(container.querySelector('.katex')).not.toBeInTheDocument()
    expect(container.textContent).toContain('0.0058')
    expect(container.textContent).toMatch(/0\.022|合计/)
  })

  it('renders subscript LaTeX without markdown italicizing underscores', () => {
    const { container } = render(
      <MarkdownRenderer content={'策略参数 $Q_{\\theta}(s,a)$ 与 $\\pi_\\phi$。'} />,
    )
    ...
    expect(container.textContent).not.toContain('Q_{')
  })

  it('demotes decorative spacing math to plain text', () => {
    const { container } = render(
      <MarkdownRenderer content={'$\\qquad$EasyRL 与 $\\text{DQN}$ 入门'} />,
    )
    expect(container.querySelector('.katex')).not.toBeInTheDocument()
    expect(container.textContent).toContain('EasyRL')
    expect(container.textContent).toContain('DQN')
    expect(container.textContent).not.toContain('\\qquad')
    expect(container.textContent).not.toContain('\\text')
  })

  it('protects unfenced ASCII residual diagrams as code (not GFM tables)', () => {
    const diagram = [
      '每一层通常类似:',
      '',
      'x',
    ...
    expect(viewer).toHaveTextContent('Attention')
    expect(viewer).toHaveTextContent('+--------------------+')
    // Should not collapse into a markdown table
