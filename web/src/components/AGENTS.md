...
编辑 `web/src/components/` 下任何文件前先读本文。它是可复用组件的权威索引、新组件的放置规则，以及样式 / i18n / 无障碍 / 测试的强制约定。

规则优先级：根 `AGENTS.md` < `web/AGENTS.md` < 本文件。冲突时以本文件为准。

配套文档：[`web/docs/component-library-plan.md`](../../docs/component-library-plan.md) 记录审计证据与后续计划。
...
| 品牌标记 haha | `BrandSeal` | `composite/BrandSeal.tsx` |
