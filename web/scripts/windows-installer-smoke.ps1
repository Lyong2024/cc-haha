...
$testRoot = Join-Path ([IO.Path]::GetTempPath()) "haha-installer-smoke-$([Guid]::NewGuid().ToString('N'))"
...
$savedEnvironment = @{}
foreach ($name in @('APPDATA', 'LOCALAPPDATA', 'USERPROFILE', 'CLAUDE_CONFIG_DIR', 'HAHA_APP_PORTABLE_DIR', 'COMPLUS_Version')) {
...
# unavailable (see HahaFindInstallProcess's findstr list). Without a CLR the
...
try {
  ...
  Remove-Item Env:HAHA_APP_PORTABLE_DIR -ErrorAction SilentlyContinue
