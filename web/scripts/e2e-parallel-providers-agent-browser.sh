...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/haha-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-haha-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-haha-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/haha" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'haha'), { recursive: true })
writeFileSync(join(configDir, 'haha', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('haha-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('haha-session-runtime', JSON.stringify({
