...
function controlledSidecarEnvironment(
  ...
  return {
    ...
    HAHA_LOCAL_ACCESS_TOKEN: localAccessToken,
    NODE_ENV: 'test',
...
describe('build-sidecars Windows x64 target mapping', () => {
  ...
  it('wires the opt-in compiled sidecar smoke into the native gate', () => {
    ...
    expect(desktopPackage.scripts?.['test:compiled-sidecar-smoke']).toContain(
      'HAHA_RUN_COMPILED_SIDECAR_SMOKE=1',
    )
  ...
  it('bounds a sidecar response with headers but a never-ending body and still cleans up', async () => {
    ...
    const originalEnvironment = {
      ...
      HAHA_LOCAL_INDEX: process.env.HAHA_LOCAL_INDEX,
      HAHA_LOCAL_ACCESS_TOKEN: process.env.HAHA_LOCAL_ACCESS_TOKEN,
    }
    const rootDir = await mkdtemp(joinPath(tmpdir(), 'haha-hung-sidecar-smoke-'))
    ...
    expect(process.env.HAHA_LOCAL_INDEX).toBe(originalEnvironment.HAHA_LOCAL_INDEX)
    expect(process.env.HAHA_LOCAL_ACCESS_TOKEN).toBe(
      originalEnvironment.HAHA_LOCAL_ACCESS_TOKEN,
    )
  })
})

const compiledSidecarSmokeEnabled =
  process.env.HAHA_RUN_COMPILED_SIDECAR_SMOKE === '1'
const configuredCompiledSidecarStarts = Number.parseInt(
  process.env.HAHA_COMPILED_SIDECAR_SMOKE_STARTS ?? '',
  10,
)
...
describe.skipIf(!compiledSidecarSmokeEnabled)('compiled sidecar local-index smoke', () => {
  it('uses SQLite by default, serves one indexed session, and reopens the database', async () => {
    ...
    const rootDir = await mkdtemp(joinPath(tmpdir(), 'haha-compiled-sidecar-smoke-'))
