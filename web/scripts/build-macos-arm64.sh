...
Usage:
  ./web/scripts/build-macos-arm64.sh [extra electron-builder args...]
...
if [[ "${SKIP_PACKAGE_SMOKE:-0}" != "1" ]]; then
  PACKAGE_SMOKE_ARGS=(bun run test:package-smoke --platform macos --package-kind release --artifacts-dir web/build-artifacts/macos-arm64)
