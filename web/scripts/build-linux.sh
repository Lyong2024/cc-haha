...
Usage:
  ./web/scripts/build-linux.sh [extra electron-builder args...]
...
if [[ "${SKIP_PACKAGE_SMOKE:-0}" != "1" ]]; then
  ...
  (cd "${REPO_ROOT}" && bun run test:package-smoke --platform linux --package-kind release --artifacts-dir "web/build-artifacts/${CANONICAL_ARCH}")
fi
