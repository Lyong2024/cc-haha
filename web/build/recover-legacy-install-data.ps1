...
param(
  ...
  [string]$ActiveConfigManaged = $env:HAHA_APP_PORTABLE_DIR,
...
function Test-LegacyPortableData {
  ...
  foreach ($childDir in @('Cache', 'EBWebView', 'projects', 'skills', 'plugins', 'cowork_plugins', 'haha')) {
...
function Run-SelfTest {
  $testRoot = Join-Path ([IO.Path]::GetTempPath()) "haha-storage-recovery-$([Guid]::NewGuid().ToString('N'))"
