...
- 变更文件：
  - `web/src/pages/TerminalSettings.tsx`
  - `web/src/pages/TerminalSettings.test.tsx`
- 验证证据：
...
- 变更文件：
  - `web/src/components/layout/TabBar.tsx`
  - `web/src/components/layout/TabBar.test.tsx`
- 验证证据：
...
- 变更文件：
  - `web/scripts/electron-output-guard.test.ts`
  - `web/electron/services/pets.test.ts`
  - `web/electron/services/sidecarManager.test.ts`
  - `web/electron/services/terminal.test.ts`
  - `web/electron/services/petWindow.test.ts`
  - `web/src/components/workspace/WorkspaceDiffSurface.test.tsx`
  - `web/src/components/workspace/WorkspacePanel.test.tsx`
- 验证证据：
...
- 变更文件：`web/electron/services/systemProxyBridge.ts`
...
- 变更文件：`web/electron/services/tray.ts`、`web/electron/services/tray.test.ts`
...
- 根因：`web/src/stores/agentStore.ts` 在已成功 create/list 后仍等待 `agentsApi.reload()`；reload 请求使用 120 秒 timeout，并在结束前保持全局 `isMutating=true`。热重载本应是保存后的非阻塞增强，失败只需 warning + Retry。
...
- 变更文件：
  - `web/src/components/chat/ContextUsageIndicator.tsx`
  - `web/src/components/chat/ContextUsageIndicator.test.tsx`
  - `web/src/api/client.ts`
  - `web/src/api/client.test.ts`
  - `web/src/stores/cliTaskStore.ts`
  - `web/src/stores/cliTaskStore.test.ts`
  - `web/electron/services/sidecarManager.ts`
  - `web/electron/services/sidecarManager.test.ts`
  - `src/server/index.ts`
...
- 排除项：当前配置选择的是仓库内置 `dada-code`，4 个内置宠物资源均存在且与 HEAD 一致。另一台电脑导入的猫/狗宠物位于 `${CLAUDE_CONFIG_DIR}/haha/pets`，属于本地用户状态，不会通过 Git 自动同步；本次故障不是资源丢失或 fallback。
...
- 变更文件：
  - `web/electron/services/petWindow.ts`
  - `web/electron/services/petWindow.test.ts`
- 验证证据：
...
- 变更文件：
  - `web/electron/services/petWindow.ts`
  - `web/electron/services/petWindow.test.ts`
  - `web/src/features/pets/PetApp.tsx`
  - `web/src/features/pets/PetApp.test.tsx`
  - `web/src/theme/globals.css`
  - `web/src/theme/globals.test.ts`
- 验证证据：
