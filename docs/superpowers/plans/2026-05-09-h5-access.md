...
**Goal:** Add opt-in personal/team H5 browser access with a generated token, controlled origin handling, authenticated REST/WebSocket access, and a mobile-safe chat shell without disrupting the default web/Tauri flow.

**Architecture:** Store H5 access configuration under the existing haha managed settings file, expose a narrow H5 settings API for the desktop Settings UI, and validate the generated token in the shared server auth path when remote access is required. The browser H5 runtime reuses the current Vite SPA with a connection screen, central REST/WebSocket auth injection, and responsive chat layout adjustments.
...
- Create `web/src/api/h5Access.ts`: typed client for H5 settings endpoints.
- Modify `web/src/api/client.ts`: auth token storage/injection helpers for browser H5 mode.
- Modify `web/src/api/websocket.ts`: `http -> ws`, `https -> wss`, and token query injection.
- Modify `web/src/lib/desktopRuntime.ts`: read/write H5 browser connection state and distinguish startup health from authenticated verification.
- Create `web/src/components/layout/H5ConnectionView.tsx`: browser-only connection form.
- Modify `web/src/components/layout/AppShell.tsx`: show H5 connection view on browser auth startup failure; keep Tauri startup unchanged.
- Modify `web/src/stores/settingsStore.ts`, `web/src/types/settings.ts`, and `web/src/pages/Settings.tsx`: Settings UI state and controls for H5 enable/token URL management.
- Modify `web/src/i18n/locales/en.ts` and `web/src/i18n/locales/zh.ts`: new H5 labels.
- Create `web/src/hooks/useMobileViewport.ts`: shared mobile breakpoint hook.
- Modify `web/src/theme/globals.css`, `web/src/components/layout/AppShell.tsx`, `web/src/components/layout/Sidebar.tsx`, `web/src/pages/ActiveSession.tsx`, `web/src/components/chat/ChatInput.tsx`, and `web/src/components/controls/PermissionModeSelector.tsx`: mobile shell and touch-target improvements.
- Add/modify desktop tests under `web/src/**`.
- Create `docs/web/h5-access.md`: concise user setup notes for LAN/reverse proxy mode.
...
describe('H5AccessService', () => {
  ...
  it('preserves unrelated managed settings fields', async () => {
    ...
    const saved = JSON.parse(await fs.readFile(path.join(tmpDir, 'haha', 'settings.json'), 'utf-8'))
...
- Persist under `haha/settings.json` in an `h5Access` object.
...
**Files:**
- Create: `web/src/api/h5Access.ts`
...
- Test: `web/src/__tests__/generalSettings.test.tsx`
...
```bash
cd web && bunx vitest run src/__tests__/generalSettings.test.tsx src/stores/settingsStore.test.ts
```
...
```bash
git add web/src/api/h5Access.ts web/src/types/settings.ts web/src/stores/settingsStore.ts web/src/pages/Settings.tsx web/src/i18n/locales/en.ts web/src/i18n/locales/zh.ts web/src/__tests__/generalSettings.test.tsx
...
- Modify: `web/src/lib/desktopRuntime.ts`
- Create: `web/src/components/layout/H5ConnectionView.tsx`
- Modify: `web/src/components/layout/AppShell.tsx`
- Test: `web/src/api/client.test.ts`
- Test: `web/src/api/websocket.test.ts`
...
```ts
const H5_SERVER_URL_KEY = 'haha-h5-server-url'
const H5_TOKEN_KEY = 'haha-h5-token'
```
...
```bash
cd web && bunx vitest run src/api/client.test.ts src/api/websocket.test.ts src/components/layout/AppShell.test.tsx
```
...
```bash
git add web/src/api/client.ts web/src/api/websocket.ts web/src/lib/desktopRuntime.ts web/src/components/layout/H5ConnectionView.tsx web/src/components/layout/AppShell.tsx web/src/api/client.test.ts web/src/api/websocket.test.ts web/src/components/layout/AppShell.test.tsx
...
**Files:**
- Create: `web/src/hooks/useMobileViewport.ts`
- Modify: `web/src/theme/globals.css`
- Modify: `web/src/components/layout/AppShell.tsx`
- Modify: `web/src/components/layout/Sidebar.tsx`
- Modify: `web/src/pages/ActiveSession.tsx`
- Modify: `web/src/components/chat/ChatInput.tsx`
- Modify: `web/src/components/controls/PermissionModeSelector.tsx`
- Test: `web/src/components/layout/AppShell.test.tsx`
- Test: `web/src/pages/ActiveSession.test.tsx`
- Test: `web/src/components/chat/ChatInput.test.tsx`
...
```bash
cd web && bunx vitest run src/components/layout/AppShell.test.tsx src/pages/ActiveSession.test.tsx src/components/chat/ChatInput.test.tsx
```
...
```bash
git add web/src/hooks/useMobileViewport.ts web/src/theme/globals.css web/src/components/layout/AppShell.tsx web/src/components/layout/Sidebar.tsx web/src/pages/ActiveSession.tsx web/src/components/chat/ChatInput.tsx web/src/components/controls/PermissionModeSelector.tsx web/src/components/layout/AppShell.test.tsx web/src/pages/ActiveSession.test.tsx web/src/components/chat/ChatInput.test.tsx
...
Create `docs/web/h5-access.md` with:
...
cd web && VITE_DESKTOP_SERVER_URL=http://127.0.0.1:3456 bun run dev -- --host 127.0.0.1 --port 1422
```
...
```bash
git add docs/web/h5-access.md docs/.vitepress/config.mts
...
Do not let multiple workers edit `web/src/components/layout/AppShell.tsx` at the same time. If parallel execution is used, Task 4 owns `AppShell` first; Task 5 rebases on that result.
...
- Ambiguity resolved: H5 config is stored in `~/.claude/haha/settings.json`, not the shared `~/.claude/settings.json`.
